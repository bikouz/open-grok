...
Background workflows — the `workflow` tool, named `.opengrok/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default** in Open Grok (upstream ships them off pending a remote feature flag Open Grok doesn't use).
