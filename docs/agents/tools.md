...
- Every `agent()` call is serviced by `host_service.rs`: it builds a `SubagentRequest` (model/agent_type/capability_mode/worktree isolation from the opts map, `owner = SubagentOwner::workflow(run_id)`) and sends `SubagentEvent::Spawn` down the same coordinator path as `task`/`agent_swarm` — so model overrides, including `antigravity:` slugs, resolve identically. `output_schema` compiles a JSON-Schema contract with fenced-JSON extraction and one corrective retry.
